...
一套 Codex-first 的 AI 开发工作流。它把“想法先发散收敛、模糊需求先澄清、异常先归因、外部事实先核对、跨层取值走正路、完成前拿证据”做成可复用的 skills 和项目 `AGENTS.md` 约束，让开发者用自然语言就能触发，不需要背阶段号。
...
| `skills/ai-task-brainstorm/SKILL.md` | 方案发散与收敛：把粗想法变成推荐路径 | 安装进 Codex/Claude skills |
...
| `安装.sh` | 一键软链四个 Skill（幂等/可卸载） | `bash 安装.sh`（旧版，推荐用 `deploy.sh`） |
...
这会把 workflow skills 安装到 `~/.agents/skills/`，生成 `ai-workflow/`，并把自然语言路由注入当前项目的 `AGENTS.md`。Codex 后续从这个业务 repo 启动时，会读取 `AGENTS.md`，并可隐式触发 `ai-task-brainstorm`、`ai-task-preflight`、`attribute-rootcause`、`verify-closure` 四个 skills。
...
| 你说的话 | 会触发的工作流 |
|---|---|
| `帮我想想这个功能怎么设计，先别写代码` | `ai-task-brainstorm` 发散方案并收敛推荐路径 |
...
1. **Day1** 启用 `ai-task-brainstorm` + `ai-task-preflight` + `verify-closure`：先把方案和需求问清楚，再立铁律“没实证不说修好了”。
2. **Day2** 完善 `ai-workflow/环境真相档案.md`，AI 每会话先读。
3. **Day3** 建立取值地图 + 执行 `bash deploy.sh --install-review-addon` 并进 `code-self-review` §6.9。

详见 `推广手册.md` §4。

## 真正新建的日常 Skill 只有 4 个

方案发散、开工准备、验证、归因。安装入口只负责把本工作流接入当前业务 repo，不参与日常编码判断。取值地图复用 `code-self-review`；真相档案是文档；回灌是写进 `AGENTS.md` / `CLAUDE.md` 的习惯。**刻意不臃肿。**
