...
`四阶段工作流.html` 是方法论介绍页，适合打开给团队讲这套方法；`四阶段使用说明.html` 是操作说明页，供同事第一次照着安装和调用。真正的落地入口是 Claude Code `/four-stage-install`，或命令行 `bootstrap.sh` / `deploy.sh`；项目内落地资产是生成到业务 repo 的 `ai-workflow/` 文档。
