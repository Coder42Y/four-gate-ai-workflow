...
```
任务 ─▶ 闸门0 归因 ─▶ 闸门1 外部真相 ─▶ 闸门2 取值地图 ─▶ 闸门3 验证闭环 ─▶ 宣称完成
...
| 文件 | 是什么 | 给谁 |
|---|---|---|
| `四闸门蓝图.md` | 规范性总览（脊柱） | 想懂全貌 |
| `推广手册.md` | 为什么/三天起步/量化胜仗/回应质疑 | 负责人、推广 |
| `环境真相档案.md` | 闸门1底座 + **取值地图本体** + 脱敏实例 + 空白模板 | 落进每个项目根 |
| `取值地图增补.md` | 闸门2：给 `code-self-review` 加 §6.9（查绕路非裸数字） | 并进现有 skill |
| `CLAUDE.md片段-回灌与闸门.md` | 四闸门常驻提醒 | 粘进项目 CLAUDE.md |
| `skills/verify-closure/SKILL.md` | 闸门3 · 验证闭环（**第一枪**） | 软链进 ~/.claude/skills |
| `skills/attribute-rootcause/SKILL.md` | 闸门0 · 归因 | 软链进 ~/.claude/skills |
...
---
来源：某同栈项目真实会话 2026-05~06，多条踩坑记忆 + 多次会话 / 数千工具调用。每条"实证"可追溯到脱敏后的案例条目。
