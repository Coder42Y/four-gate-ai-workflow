...
create_workflow_dir() {
  ...
  if [ -f "$HERE/四阶段汇报.html" ]; then
    cp "$HERE/四阶段汇报.html" "$awd/四阶段汇报.html"
    log "复制: 四阶段汇报.html"
  fi
}
...
print_summary() {
  ...
  printf '  6. 打开四阶段汇报页（可选）\n'
  printf '     → ai-workflow/四阶段汇报.html\n\n'
  printf '  7. 重启 Codex / Claude Code 会话，触发词生效:\n'
...
do_project_check() {
  ...
  [ -f "$awd/四阶段工作流.html" ] && log "四阶段工作流.html 存在" || printf '  ✗ 四阶段工作流.html 缺失\n'
  [ -f "$awd/四阶段使用说明.html" ] && log "四阶段使用说明.html 存在" || printf '  ✗ 四阶段使用说明.html 缺失\n'
  [ -f "$awd/四阶段汇报.html" ] && log "四阶段汇报.html 存在" || printf '  ✗ 四阶段汇报.html 缺失\n'
