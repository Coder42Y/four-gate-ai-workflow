...
SKILL_NAMES=(verify-closure attribute-rootcause)
...
detect_tech_stack() {
  ...
  if [ -f "$proj/pom.xml" ]; then
    ...
    # 多模块
    local modules
    modules=$(grep -c '<module>' "$proj/pom.xml" 2>/dev/null || echo "0")
  ...
  if [ -f "$proj/Jenkinsfile" ]; then
    ...
    DETECTED_CI="gitlab"
    log "检测到 GitLab CI"
  elif [ -f "$proj/.github/workflows" ]; then
    DETECTED_CI="github"
...
print_summary() {
  ...
  printf '  3. （可选）把取值地图禁用歧路加入 code-self-review §6.9\n'
  printf '     → 参考 ai-workflow/取值地图指南.md 的说明\n\n'
  printf '  4. 重启 Claude Code 会话，触发词生效:\n'
  ...
  case "$mode" in
    ...
    install)
      ...
      if [ -n "$project_path" ]; then
        ...
        printf '  → 重启 Claude Code 会话后触发词生效:\n'
        printf '    验证闭环: 「验证一下」「修好了吗」「确认生效」\n'
        printf '    归因:     「根因是什么」「先别急着改」「归因」\n'
      fi
      ;;
